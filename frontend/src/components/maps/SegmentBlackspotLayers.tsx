...
import {
  SEGMENT_BLACKSPOT_COLOR_EXPR,
  getSegmentBlackspotOpacityExpr,
  getSegmentBlackspotBgOpacityExpr,
  getSegmentBlackspotWidthExpr,
  SEGMENT_BLACKSPOT_COLORS,
} from "../../config/segmentBlackspotConfig";
...
export default function SegmentBlackspotLayers({
  filters,
  fetchFn,
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      ...
      try {
        ...
        if (mounted) {
          console.error("Failed to load segment blackspots:", err);
          ...
          setError(
            error?.response?.status
              ...
              : error?.message || "Failed to analyze segment blackspots."
          );
        }
      } finally {
  ...
  useEffect(() => {
    ...
    const onMove = (e: import("react-map-gl/maplibre").MapLayerMouseEvent) => {
      const features = map.queryRenderedFeatures(e.point, {
        layers: ["segment-blackspot-line"],
      });
      if (features.length > 0) {
        map.getCanvas().style.cursor = "pointer";
        const f = features[0];
        const segId = f.properties?.segment_id || f.properties?.corridor_id;
        
        // Update hover state if we aren't currently selecting something else
        setHoveredId(segId);
        
        if (!selectedId || selectedId === segId) {
          setHovered({
            ...
            segment_rank: f.properties?.segment_rank || f.properties?.corridor_rank,
            segment_length: f.properties?.segment_length || f.properties?.corridor_length,
          });
        }
      } else {
  ...
  const activeSegmentId = selectedId || hoveredId;
  ...
  if (!data || data.features?.length === 0) {
    return (
      <StatusBadge>
        ...
        <span>No segment blackspots found for the given criteria.</span>
      </StatusBadge>
    );
  }
  ...
  return (
    <>
      <Source id="segment-blackspots-source" type="geojson" data={data}>
        <Layer
          id="segment-blackspot-line-bg"
          type="line"
          layout={{
            "line-cap": "round",
            "line-join": "round",
          ...
          paint={{
            "line-color": "#FFFFFF",
            "line-width": getSegmentBlackspotWidthExpr(activeSegmentId, true) as any,
            "line-opacity": getSegmentBlackspotBgOpacityExpr(activeSegmentId) as any,
            "line-blur": 0,
            "line-opacity-transition": { duration: 300 },
          }}
        />
        <Layer
          id="segment-blackspot-line"
          type="line"
          layout={{
            "line-cap": "round",
            "line-join": "round",
          ...
          paint={{
            "line-color": SEGMENT_BLACKSPOT_COLOR_EXPR as any,
            "line-width": getSegmentBlackspotWidthExpr(activeSegmentId, false) as any,
            "line-opacity": getSegmentBlackspotOpacityExpr(activeSegmentId) as any,
          ...
          style={
            {
              "--popup-bg":
                SEGMENT_BLACKSPOT_COLORS[
                  hovered.priority_level as keyof typeof SEGMENT_BLACKSPOT_COLORS
                ] ?? SEGMENT_BLACKSPOT_COLORS["Moderate-Priority"],
          ...
          <div className="w-[185px] sm:w-[195px] overflow-hidden rounded-xl bg-white/95 backdrop-blur-md shadow-xl border border-slate-200/90 font-sans tracking-tight text-slate-800 select-none transition-all">
            ...
            >
              <span className="truncate max-w-[145px]">
                {hovered.priority_level} #{hovered.segment_rank ?? "-"}
              </span>
              <button
                onClick={(e) => {
                  e.stopPropagation();
            ...
            <div className="p-2 space-y-1.5 text-[10px]">
              ...
              <div>
                ...
                <div className="text-[9px] font-medium text-slate-500 flex items-center gap-1 mt-0.5">
                  ...
                  <span>
                    {(hovered.segment_length
                      ? hovered.segment_length / 1000
                      : 0
                    ).toFixed(2)}{" "}
                    km
                  </span>
