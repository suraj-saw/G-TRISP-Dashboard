/**
 * @file StateBaseMap.tsx
 * @description The foundational map component for rendering district-level spatial data.
 * @responsibility Manages the core Maplibre instance, handles viewport resizing, dynamically computes and applies inverted polygon masks to obscure areas outside the active state boundary, and renders coordinate context menus.
 */
import {
  useEffect,
  useRef,
...
export interface StateBaseMapHandle {
  resize: () => void;
  getMap: () => MapRef | null;
}
 ...
 */
const StateBaseMap = forwardRef<StateBaseMapHandle, Props>(
  (
    {
    ...
    const applyMaxBounds = useCallback(
      (bbox: [number, number, number, number] | null) => {
        ...
        // 0.5 degrees padding (~55km) for enough breathing room to avoid zooming bounce
        const pad_w = (target_w - dw) / 2 + 0.5;
        const pad_h = (target_h - dh) / 2 + 0.5;
...
StateBaseMap.displayName = "StateBaseMap";
export default StateBaseMap;
