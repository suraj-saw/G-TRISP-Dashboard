/**
 * @file SegmentBlackspotLegend.tsx
 * @description Map overlay component that displays a legend specifically for Segment Blackspots.
 * @responsibility Shows the priority levels, colors, and line thickness representation.
 */
import {
  SEGMENT_PRIORITY_LEVELS,
  SEGMENT_BLACKSPOT_COLORS,
  SEGMENT_BLACKSPOT_OPACITIES,
} from "../../config/segmentBlackspotConfig";
import type { SegmentPriorityLevel } from "../../config/segmentBlackspotConfig";

interface SegmentBlackspotLegendProps {
  visualizationLayerType?: string;
}

const LINE_WIDTHS: Record<SegmentPriorityLevel, string> = {
  "Critical Blackspot": "5px",
  "High-Priority": "3.5px",
  "Moderate-Priority": "2.5px",
};

export default function SegmentBlackspotLegend({
  visualizationLayerType,
}: SegmentBlackspotLegendProps) {
  const isVisible =
    visualizationLayerType === "segment_blackspots" ||
  ...
  return (
    <div className="absolute bottom-5 right-3 z-20 bg-white/95 backdrop-blur-sm border border-slate-200 shadow-md rounded-lg px-2.5 py-2 pointer-events-auto min-w-[155px] font-sans">
      ...
      <div className="flex flex-col gap-1.5">
        {SEGMENT_PRIORITY_LEVELS.map((level) => (
          <div
            key={level}
            ...
            <div className="flex items-center gap-2">
              <div className="w-5 flex items-center justify-center h-3">
                <div
                  ...
                  style={{
                    backgroundColor: SEGMENT_BLACKSPOT_COLORS[level],
                    height: LINE_WIDTHS[level],
                    opacity: SEGMENT_BLACKSPOT_OPACITIES[level],
                  }}
                />
              </div>
