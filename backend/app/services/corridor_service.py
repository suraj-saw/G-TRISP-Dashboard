"""
Risk Corridors Service.

Computes continuous high-risk corridors along road centerlines (e.g. NH-48)
...
def snap_corridor_gaps(
    ...
    between adjacent corridors along the same road.
    1. If two adjacent corridors have the same priority level, they merge cleanly.
    ...
    refined_corridors: List[Dict[str, Any]] = []

    for r_id, road_corrs in roads.items():
        road_len = road_lengths_map.get(r_id, 0.0)
        road_corrs.sort(key=lambda x: (x["start_m"], x["end_m"]))
        
        # Pass 1: Merge adjacent corridors of the same priority if gap <= max_snap_gap_m
        ...
        for c in road_corrs:
            ...
            if gap <= max_snap_gap_m and prev.get("priority_level") == c.get("priority_level"):
                ...
                prev["corridor_length_m"] = max(0.0, prev["end_m"] - prev["start_m"])
                ...
                eff_len = max(prev["corridor_length_m"], 500.0) / 1000.0
        ...
        # Pass 2: Snap different-priority adjacent corridors that have a small gap <= max_snap_gap_m
        for i in range(len(merged) - 1):
            ...
            if 0.0 < gap <= max_snap_gap_m:
                ...
                c1["corridor_length_m"] = max(0.0, c1["end_m"] - c1["start_m"])
                c2["start_m"] = mid_m
                c2["start_fraction"] = mid_frac
                c2["corridor_length_m"] = max(0.0, c2["end_m"] - c2["start_m"])

        refined_corridors.extend(merged)

    return refined_corridors


def compute_risk_corridors(
    db: Session,
    ...
    """
    Computes continuous risk corridors using centerline road datasets.
    Accidents within buffer_distance_m (default 100m) are snapped and analyzed along the corridor.
    """
    ...
    for idx, road_feat in enumerate(target_features):
        ...
        # 4. Merge strictly overlapping candidate segments without diluting across empty gaps
        corridors = generate_risk_corridors(
            candidates,
    ...
    ranked_corridors = rank_corridors(all_corridors, road_lengths_map)

    # 5b. Close any small gaps (<= 50m) between adjacent corridors along the same road
    seamless_corridors = snap_corridor_gaps(ranked_corridors, road_lengths_map, max_snap_gap_m=50.0)

    # 6. Extract geometries for each corridor
    features = []
    for c in seamless_corridors:
        road_id = c["road_id"]
